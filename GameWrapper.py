...
class GameWrapper:
    ...
    def play_turn(self, player_index):
        ...
        start = time.time()
        # move returns (pos, soldier, dead_opponent_pos)
    ...
    def run_game(self):
        ...
        while True:
            ...
            if self.turn_number >= 18:
                ...
                self.game.add_pos_to_board_and_update_position(
                    move=move, player_idx=player_index)

            made_mill = self.game.isMill(
                position=move[0], player_index=player_index + 1)

            if made_mill:
                assert move[2] != - \
                    1, 'illegal move, did not choose opponent soldier to take out'
            else:
                assert move[2] == - \
                    1, 'illegal move, chose opponent soldier to take out but did not make mill'

            if self.print_game_in_terminal:
                print('\nBoard after player', player_index + 1, 'moved')
                print("player", player_index + 1,
                      "moved soldier to position", move[0])
                if made_mill:
                    print(
                        "the player made a mill and killed rival soldier from cell", move[2])
                self.game.print_board_to_terminal(player_id=0)
