...
class Player(AbstractPlayer):
    def __init__(self, game_time, board=None, my_pos=None, rival_pos=None, turn=0):
        ...
        self.searchAlgo = MiniMax(_utility, _succ, _heuristic)
    ...
    def make_move(self, time_limit):
        ...
        while time.time() - start_time < time_limit/21:
        ...
        if best_move == None:
            best_move = self.make_randome_move()
                ...
                for direction in direction_list:
                    if self.board[direction] == 0:
                        ...
                        soldier_that_moved = int(
                            np.where(self.my_pos == soldier_cell)[0][0])
                        if self.is_mill(cell) == False:
                            rival_cell = - 1
                        else:
                            rival_cell = np.where(self.board == 2)[0][0]
...
def _stage_1_heuristic(state, prev_state):
    curr_heuristic = \
        _heuristic_removed_soldiers(state) * 5 + \
        _heuristic_movable_soldiers(state) + \
        _heuristic_incompleat_mills(state) * 10 + \
        _heuristic_compleated_mill(state, prev_state) * 20
...
def _stage_2_heuristic(state, prev_state):
    curr_heuristic = \
        _heuristic_removed_soldiers(state) * 5 + \
        _heuristic_movable_soldiers(state) * 10 + \
        _heuristic_incompleat_mills(state) * 5 + \
        _heuristic_compleated_mill(state, prev_state) * 30
    return curr_heuristic
